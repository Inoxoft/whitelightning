...
def export_all(model, tokenizer, label_encoder, language, max_len):
    # Save H5
    h5_path = f"models/news_classifier({language}).h5"
    ...
    onnx_path = f"models/news_classifier({language}).onnx"
    with open(onnx_path, "wb") as f:
        f.write(onnx_model.SerializeToString())
    print(f"💾 Saved ONNX model: {onnx_path}")

    # Save tokenizer
    tokenizer_path = f"models/news_classifier({language})_tokenizer.json"
    with open(tokenizer_path, "w") as f:
        json.dump(tokenizer.word_index, f)
    print(f"💾 Saved tokenizer: {tokenizer_path}")

    # Save label map
    label_map_path = f"models/news_classifier({language})_scaler.json"
...
def main():
    ...
    train_df = load_and_clean(TRAIN_PATH, ALLOWED_LABELS)
    test_df = load_and_clean(TEST_PATH, ALLOWED_LABELS)
    ...
    export_all(model, tokenizer, label_encoder, language, MAX_LEN)
