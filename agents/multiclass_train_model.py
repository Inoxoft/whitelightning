...
def train_tensorflow(
    ...
    onnx_model, _ = tf2onnx.convert.from_keras(
        full_model, input_signature=spec, opset=17
    )
...
def train_model_from_params(language, text_type, labels, platform, epochs):
    ...
    def tokenize(df):
        return pad_sequences(
            tokenizer.texts_to_sequences(df["text"]), maxlen=max_len, padding="post"
        )
    ...
    if platform == "tensorflow":
        ...
        export_model(
            model, torch.tensor(X_test[:1], dtype=torch.long), language, text_type
        )
