...
class BinaryClassifierDataGenerator:
    ...
    def __init__(
        ...
        self.analyze_performance_data_path = Path(analyze_performance_data_path) if analyze_performance_data_path else None
        ...
        logger.info(f"DataGenerator initialized. Config Model: '{self.config_model}', Data Gen Model: '{self.selected_data_gen_model}'")
        logger.info(f"Prompt Refinement Cycles: {self.prompt_refinement_cycles}")
        logger.info(f"Generate Edge Cases: {self.generate_edge_cases} (Target Volume: {self.edge_case_volume})")
        if self.analyze_performance_data_path:
            logger.info(f"Performance analysis requested using data from: {self.analyze_performance_data_path}")


    async def _call_llm_async(self, model: str, system_prompt: str, user_prompt: str, max_tokens: int = 4000, temperature: float = 0.7) -> str:
    ...
    async def _generate_initial_config_async(self) -> bool:
        ...
        logger.info(f"Generating initial configuration using model: {self.config_model}")
        ...
        try:
            ...
            # Basic validation
            required_keys = ["summary", "prompts", "model_prefix", "training_data_volume"]
            if not all(key in config_data for key in required_keys):
                raise ValueError(f"Generated config missing keys. Expected: {required_keys}, Got: {config_data.keys()}")
            ...
            self.final_config['parameters'] = self._get_init_parameters() # Store initial params
            return True
            ...
            logger.error(f"Failed to generate initial configuration: {e}", exc_info=True)
            return False
    ...
    def _get_init_parameters(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analyze_performance_data_path": str(self.analyze_performance_data_path) if self.analyze_performance_data_path else None,
        }
    ...
    def _prepare_output_directory(self):
        ...
        self.edge_case_dataset_path = self.model_output_path / settings.EDGE_CASE_DATASET_FILENAME
    ...
    def _save_raw_response(self, prompt_type: str, prompt: str, response: str, label: Optional[int], model: str, status: str = "success"):
        """Saves raw LLM response to a JSON file."""
        if not self.raw_responses_path: return

        ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        label_str = f"_{settings.POSITIVE_LABEL}" if label == settings.POSITIVE_LABEL else f"_{settings.NEGATIVE_LABEL}" if label == settings.NEGATIVE_LABEL else ""
        safe_model = model.replace('/', '_').replace(':', '_')
        filename = self.raw_responses_path / f"{prompt_type}{label_str}_{safe_model}_{status}_{ts}.json"

        data = {
            "timestamp": datetime.now().isoformat(), "model_used": model,
            "prompt_type": prompt_type, "label": label, "status": status,
        ...
        try:
            ...
            logger.warning(f"Failed to save raw response to {filename}: {e}", exc_info=False)
        ...
        try:
            ...
            with open(target_path, 'a', encoding='utf-8', newline='') as f:
                ...
                for line in text_data.split('\n'):
                    ...
                    if len(cleaned_line) > 1 and cleaned_line.startswith('"') and cleaned_line.endswith('"'):
            ...
            return 0
             ...
             logger.error(f"Unexpected error appending to {target_path}: {e}", exc_info=True)
             return 0

    async def _generate_text_samples_batch_async(self, prompts_labels: List[Tuple[str, int, str]], model: str, system_prompt: str) -> List[Tuple[str, int, str, str]]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                ...
                logger.error(f"Data generation failed for {prompt_type} (Label: {label}) - {error_msg}")
                self._save_raw_response(prompt_type, prompt, error_msg, label, model, status="failed")
                processed_results.append((prompt, label, error_msg, "failed"))
            else:
                self._save_raw_response(prompt_type, prompt, result, label, model, status="success")
    ...
    async def _run_prompt_refinement_cycle_async(self, cycle_num: int) -> bool:
        ...
        if not self.final_config: return False
        logger.info(f"--- Starting Prompt Refinement Cycle {cycle_num + 1}/{self.prompt_refinement_cycles} ---")

        current_prompts = self.final_config['prompts']
        pos_prompt = current_prompts['positive']
        neg_prompt = current_prompts['negative']
        data_gen_sys_prompt = settings.DATA_GEN_SYSTEM_PROMPT
        ...
        for _ in range(num_samples_per_type):
             sample_prompts_labels.append((pos_prompt, settings.POSITIVE_LABEL, "refinement_sample"))
             sample_prompts_labels.append((neg_prompt, settings.NEGATIVE_LABEL, "refinement_sample"))
        ...
        pos_samples = [res[2] for res in sample_results if res[1] == settings.POSITIVE_LABEL and res[3] == "success"]
        neg_samples = [res[2] for res in sample_results if res[1] == settings.NEGATIVE_LABEL and res[3] == "success"]

        if not pos_samples and not neg_samples:
             logger.warning(f"Refinement Cycle {cycle_num + 1}: Failed to generate any sample data. Skipping refinement.")
             return False # Cannot refine without samples

        pos_samples_str = "\n".join([f"- {s[:200]}..." for s in pos_samples[:5]]) # Show first 5 samples (truncated)
        ...
        refinement_prompt = settings.PROMPT_REFINEMENT_TEMPLATE.format(
            ...
            positive_samples=pos_samples_str if pos_samples else "No positive samples generated.",
            negative_samples=neg_samples_str if neg_samples else "No negative samples generated."
        )
        ...
        try:
            ...
            if "evaluation_summary" not in refinement_data or "refined_positive_prompt" not in refinement_data or "refined_negative_prompt" not in refinement_data:
            ...
            logger.info(f"Refinement Cycle {cycle_num + 1} Evaluation: {refinement_data['evaluation_summary']}")
            ...
            logger.error(f"Refinement Cycle {cycle_num + 1}: Failed to parse JSON response from config model: {e}\nRaw response:\n{refinement_response}", exc_info=True)
            return False
        except (ValueError, OpenAIError, Exception) as e:
            logger.error(f"Refinement Cycle {cycle_num + 1}: Failed: {e}", exc_info=True)
            return False

    async def _generate_training_data_async(self):
        """Generates the main training dataset using final prompts."""
        if not self.final_config or not self.dataset_path: return 0
        ...
        target_volume = self.final_config.get('training_data_volume', settings.DEFAULT_TRAINING_DATA_VOLUME)
        data_gen_sys_prompt = settings.DATA_GEN_SYSTEM_PROMPT
        ...
        num_batches = max(1, (required_requests + self.batch_size - 1) // self.batch_size)
        ...
        for batch_num in range(num_batches):
            ...
            for i in range(self.batch_size):
                label = settings.POSITIVE_LABEL if (current_request_count + i) % 2 == 0 else settings.NEGATIVE_LABEL
                prompt = positive_prompt if label == settings.POSITIVE_LABEL else negative_prompt
            ...
            for _, label, response_content, status in batch_results:
                if status == "success":
                    lines_added = self._append_to_dataset(response_content, label, self.dataset_path)
        ...
        logger.info(f"Total training API requests: {total_requests_made} (Failed: {total_failed_requests})")
    ...
    async def _generate_edge_cases_async(self):
        ...
        if not self.final_config or not self.edge_case_dataset_path: return 0
        ...
        num_batches = max(1, (required_requests + edge_batch_size - 1) // edge_batch_size)

        logger.info(f"Estimated edge case API calls: {required_requests} ({num_batches} batches of size {edge_batch_size})")

        current_request_count = 0
        for batch_num in range(num_batches):
            if total_samples_generated >= target_volume:
                 logger.info(f"Target edge case volume reached ({total_samples_generated}/{target_volume}). Stopping generation.")
                 break
            ...
            for i in range(edge_batch_size):
                label = settings.POSITIVE_LABEL if (current_request_count + i) % 2 == 0 else settings.NEGATIVE_LABEL
                prompt = pos_edge_case_prompt if label == settings.POSITIVE_LABEL else neg_edge_case_prompt
            ...
            for _, label, response_content, status in batch_results:
                 if status == "success":
                     lines_added = self._append_to_dataset(response_content, label, self.edge_case_dataset_path)
                     batch_samples_added += lines_added
                 else:
                     total_failed_requests += 1

            total_samples_generated += batch_samples_added
            logger.info(f"Batch {batch_num + 1} completed. Added {batch_samples_added} edge cases. Total: {total_samples_generated}. Failed requests: {sum(1 for r in batch_results if r[3]=='failed')}")
        ...
        logger.info(f"Total edge case API requests: {total_requests_made} (Failed: {total_failed_requests})")
    ...
    async def _analyze_performance_async(self):
        ...
        logger.info(f"--- Starting Performance Analysis using {self.analyze_performance_data_path} ---")

        try:
            if not self.analyze_performance_data_path.exists():
                 raise FileNotFoundError(f"Performance results file not found at {self.analyze_performance_data_path}")
            ...
            try:
                ...
                if pd.api.types.is_numeric_dtype(df[true_label_col]) and pd.api.types.is_numeric_dtype(df[pred_col]):
                     ...
                     df['correct'] = (df[true_label_col] == df[pred_col]) if not (df[pred_col].between(0, 1, inclusive='neither')).any() \
                     ...
                     if 'correct' in df.columns:
                        ...
                        correct_df = df[df['correct']].head(limit_samples - len(errors_df))
                ...
                for _, row in sample_df.iterrows():
                    sample_results_list.append(f'- "{str(row[text_col])[:150]}...", True: {row[true_label_col]}, Predicted: {row[pred_col]}')
            except Exception as e:
                logger.error(f"Error reading performance data file: {e}", exc_info=True)
                # Provide fallback message to LLM if reading fails
                sample_results_list.append("Error: Could not read or parse the performance data file.")
            ...
            logger.info("Asking config model for performance analysis and recommendations...")
            ...
            logger.info("--- Performance Analysis Finished ---")
             ...
             logger.error(f"Performance analysis failed unexpectedly: {e}", exc_info=True)
    ...
    def _save_final_config(self):
        ...
        if not self.final_config or not self.model_output_path:
            logger.error("Cannot save final config: Configuration or output path missing.")
            return False
        if not self.final_config_path:
             self.final_config_path = self.model_output_path / settings.CONFIG_FILENAME # Ensure path exists
        ...
        self.final_config['initial_config'] = self.initial_config # Store the very first config
        ...
        self.final_config['output_paths'] = {
            ...
            'training_data': str(self.dataset_path) if self.dataset_path and self.dataset_path.exists() else None,
            'edge_case_data': str(self.edge_case_dataset_path) if self.edge_case_dataset_path and self.edge_case_dataset_path.exists() else None,
        ...
        try:
            ...
            return True
             ...
             logger.error(f"Failed to serialize final config to JSON: {e}. Check for non-serializable types.", exc_info=True)
             return False
        except IOError as e:
    ...
    async def generate_data_and_train_model_async(self, model_type: str):
        ...
        logger.info(f"=== Starting Data Generation Process for: '{self.problem_description}' ===")
        ...
        try:
            ...
            if self.prompt_refinement_cycles > 0:
                for i in range(self.prompt_refinement_cycles):
                    refinement_success = await self._run_prompt_refinement_cycle_async(i)
                    if not refinement_success:
                        logger.warning(f"Prompt refinement cycle {i+1} failed or was skipped.")
            ...
            logger.critical(f"Data generation process failed critically: {e}", exc_info=True)
        except Exception as e:
            logger.critical(f"An unexpected critical error occurred during generation: {e}", exc_info=True)
        finally:
            ...
            if self.final_config_path:
                logger.info(f"  Find detailed configuration and results in: {self.final_config_path}")
            else:
...
async def cli_main():
    ...
    parser.add_argument(
        "-p", "--problem-description", required=True,
        help="Describe the classification problem."
    )
    parser.add_argument(
        "-m", "--model", default=settings.DEFAULT_DATA_GEN_MODEL,
        help="OpenRouter model name for bulk data generation."
    )
    parser.add_argument(
        "--config-model", default=settings.DEFAULT_CONFIG_MODEL,
        help="OpenRouter model for config, refinement, edge cases, analysis (should be capable)."
    )
    parser.add_argument(
        "-l", "--library", choices=['pytorch', 'tensorflow', 'scikit'], default=None,
        help="Specify the target ML library (optional, for documentation)."
    )
    parser.add_argument(
        "-o", "--output-path", default=settings.DEFAULT_OUTPUT_PATH,
        help="Base directory for output."
    )
    parser.add_argument(
        "--refinement-cycles", type=int, default=settings.DEFAULT_PROMPT_REFINEMENT_CYCLES,
        help="Number of prompt refinement cycles based on sample data."
    )
    parser.add_argument(
        "--edge-cases", type=lambda x: (str(x).lower() == 'true'), # Handle boolean
